/* ============================================================================
   content.js — Wiskunde met Storm, H8 "Herleiden en machten" (Getal & Ruimte 1hv)
   Swapbare laag op de canonieke engine (tools/oefen-engine/). Zie PLAYBOOK + plan.

   Machine-gecheckte antwoorden (t:"bouw"/"stap"/"velden") in ^-notatie:
     machten met ^ (x^2, 10^11) · maal met * of · · NL-komma decimaal (8,75) ·
     variabelen = losse letters (5a = 5*a). equiv() rekent ze numeriek na.
   Vraagtypes: vb (voordoen) · stap (stap-voor-stap + hints) · bouw (zelf opbouwen)
     · velden (mantisse/exponent los) · mc (concept/misconceptie).
   ========================================================================== */

window.VAK = {
  id:"wiskunde_h8",
  titel:"Wiskunde met Storm — H8 Herleiden en machten",
  brand:"Wiskunde met Storm", badge:"∑", mascotEmoji:"🦦",
  spiekTitel:"Wiskunde H8 — Herleiden en machten",
  wiskunde:true,
  spraak:{ vraagLang:"nl-NL", antwoordLang:"nl-NL" },
  matching:{ stripInfinitief:false, accentTolerant:true, lidwoordTolerant:false }, // accent: 'coëfficiënt'
  masteryHits:2,
  tegels:["leerstof","begrippen","proeftoets","spiekblad"]
};

/* ---------------- LEERSTOF: de onderwerpen (8.1–8.5), elk 3 niveaus ---------------- */
window.LEERSTOF = [

/* ============================ 8.1 HERLEIDEN ============================ */
{
  id:"herleiden", emoji:"➕", titel:"8.1 Herleiden",
  kort:"Gelijksoortige termen (zelfde letter) samennemen; eerst ×, dan +/−.",
  niveaus:[
    /* ---- Brons: herkennen + voordoen ---- */
    { n:1, naam:"Brons",
      uitleg:[
      ...
      voorbeelden:[ {en:"2b + 3b = 5b", let:"Allebei 'b' → tel 2 + 3 = 5, de b blijft staan."},
                    {en:"4a + a = 5a", let:"a is hetzelfde als 1a, dus 4a + 1a = 5a."} ],
      vragen:[
        {t:"sleep", opgave:"3a + 2b + 5a + b",
          chips:[{t:"3a",bak:"a"},{t:"2b",bak:"b"},{t:"5a",bak:"a"},{t:"b",bak:"b"}],
          bakjes:[{id:"a",label:"a-termen",som:"8a"},{id:"b",label:"b-termen",som:"3b"}],
          resultaat:"8a + 3b", w:"Termen met dezelfde letter horen in hetzelfde mandje."},
        {t:"vb", opgave:"3a + 5a", waarom:{ q:"Waarom mag 3a + 5a samen tot 8a?",
            o:["Het zijn gelijksoortige termen (allebei a)","Omdat optellen altijd mag","Omdat 3 + 5 = 8"],
            a:"Het zijn gelijksoortige termen (allebei a)"},
          stappen:[
            {toon:"3a + 5a", uitleg:"3a en 5a zijn gelijksoortig: allebei de letter a."},
            {toon:"= (3 + 5)a", uitleg:"Tel de getallen vóór de a op."},
            {toon:"= 8a", uitleg:"De a blijft gewoon staan."} ]},
        {t:"mc", q:"Welke twee termen zijn gelijksoortig?",
          o:["4x en 7x","4x en 7y","4x en 7"], a:"4x en 7x",
        ...
        {t:"stap", opgave:"2b + 3b", vars:["b"], w:"2 + 3 = 5, en de b blijft staan.",
          meer:"2b betekent 2·b. 2b + 3b = (2 + 3)·b = 5b.",
          stappen:[ {prompt:"Neem de gelijksoortige termen samen", a:"5b",
            hints:["2b en 3b zijn allebei 'b' → tel 2 + 3","2 + 3 = 5, dus 5b"]} ]},
        {t:"mc", q:"Wat is 5x + 3? Let op!",
          o:["5x + 3 (kan niet korter)","8x","8"], a:"5x + 3 (kan niet korter)",
          ...
          fout:{ "8x":"je telde de 5 en de 3 samen, maar de 3 heeft geen x. Alleen termen mét x mag je optellen.",
                 "8":"je telde 5 + 3 op en liet de x weg — maar de x hoort bij de 5x en blijft staan." }},
        {t:"stap", opgave:"4a + 2 + a", vars:["a"], w:"4a + a = 5a; de 2 blijft los.",
          meer:"a is 1a, dus 4a + a = 5a. De 2 is geen a → die blijft staan: 5a + 2.",
          stappen:[ {prompt:"Neem de a-termen samen (let op: a = 1a)", a:"5a", hints:["4a + a = 4a + 1a","4 + 1 = 5 → 5a"]},
                    {prompt:"Schrijf nu de hele herleiding op", a:"5a + 2", hints:["De 2 hoort er los bij","5a + 2"]} ]}
      ]
    },
    /* ---- Zilver: zelf doen, eerst producten uitrekenen ---- */
    { n:2, naam:"Zilver",
      uitleg:[
      ...
      voorbeelden:[ {en:"−3·5x + 8·2x = −15x + 16x = x", let:"Eerst −15x en 16x, dán samen: 1x = x."} ],
      vragen:[
        {t:"stap", opgave:"−3·5x + 8·2x", vars:["x"], w:"Eerst keer, dan samennemen.",
          meer:"−3·5x = −15x en 8·2x = 16x. Dan −15x + 16x = 1x = x.",
          stappen:[ {prompt:"Reken eerst de twee producten uit", a:"-15x + 16x", hints:["−3·5x = −15x","8·2x = 16x"]},
                    {prompt:"Neem nu samen", a:"x", hints:["−15x + 16x = (−15 + 16)x","−15 + 16 = 1, dus 1x = x"]} ]},
        {t:"bouw", q:"Herleid: 5x − 2y − 3y − 5x", a:"-5y", vars:["x","y"],
          w:"Neem de x'en samen en de y's samen.",
          meer:"5x − 5x = 0. −2y − 3y = −5y. Samen blijft over: −5y."},
        {t:"bouw", q:"Herleid: 7a + 3b − 2a + b", a:"5a + 4b", vars:["a","b"],
          w:"a's bij a's, b's bij b's.",
          meer:"7a − 2a = 5a en 3b + b = 4b → 5a + 4b."},
        {t:"mc", q:"Wat is 4a + 2 + a?", o:["5a + 2","6a","7a"], a:"5a + 2",
          w:"4a + a = 5a; de 2 blijft los.",
          meer:"a = 1a, dus 4a + a = 5a. De 2 is geen a → 5a + 2."}
      ]
    },
    /* ---- Goud: door elkaar, meerdere letters + producten ---- */
    { n:3, naam:"Goud",
      uitleg:[
      ...
      voorbeelden:[ {en:"5a − 2b − 3a + 5b = 2a + 3b", let:"5a−3a=2a en −2b+5b=3b."} ],
      vragen:[
        {t:"stap", opgave:"5a − 2b − 3a + 5b", vars:["a","b"], w:"a's bij a's, b's bij b's.",
          meer:"5a − 3a = 2a en −2b + 5b = 3b → 2a + 3b.",
          stappen:[ {prompt:"Neem de a-termen samen", a:"2a", hints:["5a − 3a = 2a"]},
                    {prompt:"Neem de b-termen samen", a:"3b", hints:["−2b + 5b = 3b"]},
                    {prompt:"Schrijf het hele antwoord", a:"2a + 3b", hints:["Zet de a-term en b-term samen","2a + 3b"]} ]},
        {t:"bouw", q:"Herleid: 2x · 3 − 4 · 2x", a:"-2x", vars:["x"],
          w:"Eerst de producten: 2x·3 = 6x en 4·2x = 8x.",
          meer:"2x·3 = 6x, 4·2x = 8x. Dan 6x − 8x = −2x."},
        {t:"bouw", q:"Herleid: 3p + 5q − p − 6q + 2p", a:"4p - q", vars:["p","q"],
          w:"p's: 3p − p + 2p; q's: 5q − 6q.",
          meer:"3p − p + 2p = 4p en 5q − 6q = −q → 4p − q."},
        {t:"mc", q:"Welke herleiding klopt?",
          o:["6a − 2a = 4a","6a − 2a = 4","6a − 2a = 8a"], a:"6a − 2a = 4a",
          w:"6 − 2 = 4, de a blijft staan.",
          meer:"6a − 2a = (6 − 2)a = 4a. Niet 4 (de a blijft) en niet 8a (je telt niet op).",
          fout:{ "6a − 2a = 4":"je liet de a vallen. De a blijft staan: 4a.",
                 "6a − 2a = 8a":"je telde 6 en 2 op, maar het is aftrekken: 6 − 2 = 4." }}
      ]
    }
  ]
},
...
/* ---------------- BEGRIPPEN: vakwoorden (flashcards + spaced repetition) ---------------- */
window.BEGRIPPEN = {
  "Begrippen": { emoji:"📐", titel:"Begrippen H8", items:[
    ["het getal dat je keer zichzelf doet (bij 2⁵ is dit de 2)","grondtal"],
    ["het kleine getal bovenaan; zegt hoe vaak (bij 2⁵ is dit de 5)","exponent"],
    ["een grondtal met een exponent, zoals 2⁵","macht"],
    ["een los stukje van een som, gescheiden door + of −","term"],
    ["het getal vóór de letter (in 7a is dit de 7)","coëfficiënt"],
    ["termen met precies dezelfde letter(s); die mag je samennemen","gelijksoortig"],
    ["een som zo kort mogelijk schrijven","herleiden"],
    ["waarmee je keer doet om te groeien (×1,05 = 5% erbij)","groeifactor"]
  ]}
};

/* ---------------- OPSTAP: korte voorkennis-checks (Fase C vult deze verder) ---------------- */
window.OPSTAP = {};

/* ---------------- PROEFTOETS: gemengde toets-opgaven (Fase D vult deze) ---------------- */
window.PROEFTOETS = [];
